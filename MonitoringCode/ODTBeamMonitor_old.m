...
maxTime=2*50;  %SET THIS FOR LENGTH OF INDIVIDUAL SCANS
theCLim=[1 256];
...
% % %      relWidth = 0.5250;
    ...
    function TimerStartFcn(handle, callbackdata)
       whitebg(hFig, 'white');
       handle.UserData = struct('iter',1, 'times',zeros(100000,1), 'xNumCountsVec',zeros(100000,1),'hxplot',plot(hXPosAxes,[0],[0],'-ob') ...
           ,'yNumCountsVec',zeros(100000,1),'hyplot',plot(hYPosAxes,[0],[0],'-ob'));
    end
...
%                 if (ttlcounts > 10000)&&(ttlcounts<40000)
%                 xo = 0;
                ud.times(ud.iter) = (now-startTime)*24*60*60;
%                 ud.xNumCountsVec(ud.iter)=xo;
                ud.yNumCountsVec(ud.iter)=ttlcounts;
...
%                 ud.hxplot = plot(hXPosAxes,ud.times(1:ud.iter),ud.xNumCountsVec(1:ud.iter),'-r');  %Plotting the total integral of the light count.  i.e. Photodiode count
                ud.hyplot = plot(hYPosAxes,ud.times(1:ud.iter),ud.yNumCountsVec(1:ud.iter),'-r');  %Plotting the total integral of the light count.  i.e. Photodiode count
%                 set(ud.hxplot,'LineWidth',4)
                set(ud.hyplot,'LineWidth',4)
                annotation('textbox',[0.27 0 0.3 0.3], 'String', num2str(ud.yNumCountsVec(ud.iter),3), 'FitBoxToText','on','FontSize',32); %Display currently plotted value on graph as an annotation
%                 annotation('textbox',[0.27 0.5 0.3 0.3], 'String', num2str(ud.xNumCountsVec(ud.iter),3), 'FitBoxToText','on','FontSize',32);
                if (now-startTime)*24*60*60>maxTime
                    
                    %Getting the data from the photodiode plot
                    do=get(hXPosAxes,'Children');
%                     xdata=get(do,'XData');
                    ydata=get(do,'YData');
                    
                    
                    %Saving the Photodiode data to a file
                    save([datestr(now,'yyyy_mm_dd_HH_MM_SS') '_livePD.mat'],'xdata','ydata');  
...
%         vid = videoinput('gige', 1, 'Mono12Packed'); %use 12-bit ADC, Packed?
        vid = videoinput('gige', 1, 'Mono8'); %use 12-bit ADC, Packed?
