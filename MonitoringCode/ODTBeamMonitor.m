function [] = ODTBeamMonitor( ROIName )
...
maxTime=500;  %SET THIS FOR LENGTH OF INDIVIDUAL SCANS
theCLim=[1 256];
...
% Initialize a timer object to set the timing between camera images
myTimer=timer;
...
% What the program does when the figure is closed.
...
% What the timer object does when it first starts.
function TimerStartFcn(handle, callbackdata)
   whitebg(hFig, 'white');
   handle.UserData = struct('iter',1, 'times',zeros(100000,1), 'xCenterPositionVec',zeros(100000,1),'hxplot',plot(hXPosAxes,[0],[0],'-ob') ...
       ,'yCenterPositionVec',zeros(100000,1),'hyplot',plot(hYPosAxes,[0],[0],'-ob'));
end
   
% What the timer function does on each iteration -- first it triggers the camera, then it does data analysis and plotting 
 function TimerFcn(handle,callbackdata)
     if isequal(vid.Running,'on')
        ...
        if vid.FramesAvailable>0 
            ...
            % (timerObject).UserData is data that is saved and used in
            ...
            % Total counts for determining if an image 
%                 ttlcounts = sum(sum(img))
            ...
            [~,xo,wx,yo,wy,fi,offset]=fit2DGauss(ROI,img,cm);
            ud.times(ud.iter) = (now-startTime)*24*60*60;
            ud.xCenterPositionVec(ud.iter)=xo;
            ud.yCenterPositionVec(ud.iter)=yo;
            cla(hXPosAxes);  % Clearing axes to prevent from wasting memory
            cla(hYPosAxes); 
            delete(findall(gcf,'type','annotation')) %Delete the display of current value
            ud.hxplot = plot(hXPosAxes,ud.times(1:ud.iter),ud.xCenterPositionVec(1:ud.iter),'-r');  %Plotting X centers
            ud.hyplot = plot(hYPosAxes,ud.times(1:ud.iter),ud.yCenterPositionVec(1:ud.iter),'-r');  %Plotting Y centers
            set(ud.hxplot,'LineWidth',4)
            set(ud.hyplot,'LineWidth',4)
            annotation('textbox',[0.27 0 0.3 0.3], 'String', [num2str(ud.yCenterPositionVec(ud.iter),3),'    wy = ',num2str(wy,4),'    off = ',num2str(offset,3)], 'FitBoxToText','on','FontSize',32); %Display currently plotted value on graph as an annotation
            annotation('textbox',[0.27 0.5 0.3 0.3], 'String', [num2str(ud.xCenterPositionVec(ud.iter),3),'    wx = ',num2str(wx,4)], 'FitBoxToText','on','FontSize',32);
...
function [Amp,xo,wx,yo,wy,fi,offset] = fit2DGauss(ROI,img,cm)
    ...
    ub = double([guess(1).*2,max([ROI(3) ROI(4)]),guess(3).*2,max([ROI(1) ROI(2)]),guess(5).*2,pi./4,80]);
...
%INITIALIZECAMERAFREERUN Creates a camera object and creates settings using
